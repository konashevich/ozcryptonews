...
CSV_FILENAME = "articles_coindesk.csv"
...
def setup_driver():
    ...
    try:
        ...
        options.add_argument('--headless') # Uncommented: Run headless for background operation
    ...
    try:
        ...
        if page_source:
            ...
            if new_articles:
                ...
                append_to_csv(CSV_FILENAME, new_articles, HEADERS, SOURCE_NAME)
            else:
                 # Handle message based on whether extraction failed or just no new articles
                 if not all_extracted_articles and os.path.exists("debug_page.html"):
                     # Message about debug file already printed in extract_articles
                     pass
                 elif not all_extracted_articles:
                     print("Extraction failed or no articles found on page.")
                 else: # Articles extracted, but none were new/valid
